...
class GameServiceTest {
  ...
  private final GameArticleVersionRepository versionRepository =
      mock(GameArticleVersionRepository.class);
  ...
  private final GameService service = new GameService(
      gameRepository, versionRepository, userRepository, converter, kafka);
  ...
  void createGame_should_work() {
    ...
    verify(gameRepository, atLeastOnce())
        .save(argThat(model -> model.getStatus() == GameStatus.TO_REVIEW));
  }
  ...
  void getGames_should_return_list() {
    ...
    when(gameRepository.findAllByStatus(GameStatus.OK))
        .thenReturn(List.of(new GameModel()));
    when(converter.fromModelToEntity(any())).thenReturn(new GameEntity());
  ...
  void getGamesToReview_should_return_list() {
    when(gameRepository.findAllByStatus(GameStatus.TO_REVIEW))
        .thenReturn(List.of(new GameModel()));
    when(converter.fromModelToEntity(any())).thenReturn(new GameEntity());
  ...
  void updateGame_should_change_name_if_different() {
    ...
    when(gameRepository.findById(gameId)).thenReturn(Optional.of(gameModel));
    when(userRepository.findById(userId)).thenReturn(Optional.of(user));
    when(gameRepository.findBySubjectGameName("NEW_NAME"))
        .thenReturn(Optional.empty());
    when(converter.fromModelToEntity(any())).thenReturn(new GameEntity());
